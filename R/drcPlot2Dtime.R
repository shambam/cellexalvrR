...
setMethod('drcPlots2Dtime', signature = c ('cellexalvrR'),
	definition = function ( cellexalObj, gInfo ) {
		...
		grDevices::png( file= DRC1, width=1000, height=1000)
   		...
   		p= prettyPlot2Dtime( data.frame(x=	drc[,1], y=	drc[,2]),
   		 color(gInfo$time,colnames(cellexalObj@data)) ) #function definition in file drcPlot2D.R
    	print(p)
		dev.off()
		...
		if ( ! var(cellexalObj@drc[[gInfo$drc]][,3]) == 0 ) {
			...
			p= prettyPlot2D( data.frame(x=	drc[,1], y=	drc[,3]), color(gInfo$time,colnames(cellexalObj@data)) ) #function definition in file drcPlot2D.R
    		print(p)
			dev.off()
		}
