...
if (! file.exists( args[1]) ) {
	message( "this script needs a cellexal object file at startup")
	quit(status=1)
}

message( paste('fast walthrough using cellexal file ', args[1] ) )
...
message( paste( "set test outpath to", cellexalObj@outpath))

## save this file 
lockedSave( cellexalObj )

if ( ! file.exists(file.path(cellexalObj@outpath, "cellexalObj.RData"))){
	message ( "ERROR - file save not working" )
}
